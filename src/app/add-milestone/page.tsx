...
const milestoneSchema = z.object({
  ...
  percentage: z.coerce.number().int().positive("Percentage must be a positive whole number.").max(100, "Percentage cannot exceed 100."),
});
...
export default function AddMilestonePage() {
  ...
  async function onSubmit(data: MilestoneFormValues) {
    ...
    try {
        ...
        const dryRunResult = await suiClient.dryRunTransactionBlock({ transactionBlock: txBytes });
        if (dryRunResult.effects.status.status !== 'success') {
            const errorMsg = dryRunResult.effects.status.error || "Dry run failed with an unknown error.";
            console.error("Transaction dry run failed:", errorMsg);
            toast({ title: "Transaction Validation Failed", description: `Error: ${errorMsg}`, variant: "destructive" });
            return;
        }
        ...
        signAndExecute(
            ...
            {
                onSuccess: (result) => {
                    toast({ title: "Milestone Added Successfully!", description: `Transaction Digest: ${result.digest}` });
                },
                onError: (error) => {
  ...
  return (
    <ClientOnly>
        <div className="max-w-2xl mx-auto">
            ...
            <h1 className="text-3xl font-bold tracking-tight mb-2">Add a New Milestone</h1>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                render={({ field }) => (
                    ...
                    <FormControl>
                        <Textarea rows={5} placeholder="Describe the milestone in detail..." {...field} />
                    </FormControl>
                    <FormMessage />
                    </FormItem>
                )}
                ...
                render={({ field }) => (
                    ...
                    <FormDescription>
                        The percentage of the total goal to be released upon verification.
                    </FormDescription>
                    <FormMessage />
                    </FormItem>
                )}
