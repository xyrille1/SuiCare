...
import { Campaign, Milestone, NewCampaignData, UpdateCampaignData } from '@/lib/types';
...
export const SUI_ADMIN_CAP_ID = process.env.NEXT_PUBLIC_SUI_ADMIN_CAP_ID;
...
interface CampaignContextType {
  ...
  updateCampaign: (campaign: UpdateCampaignData, onSuccess?: () => void) => void;
...
const campaignFromSuiObject = (obj: SuiObjectData): Omit<Campaign, 'icon' | 'color' | 'iconColor'> | null => {
    if (obj.content?.dataType !== 'moveObject') {
        console.warn('Invalid object type, expected moveObject but got:', obj.content?.dataType);
        return null;
    }
    ...
    try {
        const milestones = fields.milestones.map((m): Milestone => {
            if (!m.fields || typeof m.fields.description !== 'string' || typeof m.fields.percentage !== 'string' || typeof m.fields.status !== 'number') {
                throw new Error('Invalid milestone structure');
            }
            const percentage = parseInt(m.fields.percentage, 10);
            if (isNaN(percentage)) {
                throw new Error(`Invalid percentage in milestone: ${m.fields.percentage}`);
            }
            return {
        ...
        const goal = parseInt(fields.target_amount, 10);
        const raised = parseInt(fields.donated_amount, 10);
        
        // Robustly parse escrow balance
        const escrowBalance = (typeof fields.escrow === 'object' && fields.escrow !== null && 'value' in fields.escrow)
            ? parseInt((fields.escrow as { value: string }).value, 10)
            : 0;

        if (isNaN(goal) || isNaN(raised) || isNaN(escrowBalance)) {
        ...
        return {
            ...
            adminAddress: fields.admin,
            escrowBalance: escrowBalance,
            milestones: milestones,
        };
    } catch (e: any) {
        console.error(`Failed to parse campaign object ${fields.id?.id}:`, e.message, { fields });
        return null;
    }
};
...
export function CampaignProvider({ children }: { children: ReactNode }) {
    ...
    const fetchCampaigns = useCallback(async () => {
        ...
        try {
            ...
            const { data } = await suiClient.getDynamicFields({ parentId: SUI_CAMPAIGNS_ID });
            ...
            setError(`Could not load campaign data from the network. ${errorMessage}`);
    ...
    const addCampaign = async (data: NewCampaignData, onSuccess?: () => void) => {
        ...
        try {
            ...
            signAndExecute(
                { transactionBlock: txb, options: { showEffects: true, showEvents: true } },
                {
                    onSuccess: (result) => {
                        toast({ 
    ...
    const updateCampaign = async (data: UpdateCampaignData, onSuccess?: () => void) => {
      if (!currentAccount) {
          toast({ 
              title: "Wallet Not Connected", 
      ...
      if (!SUI_ADMIN_CAP_ID) {
          toast({ 
              title: "Admin Cap ID Not Found", 
              description: "The SUI_ADMIN_CAP_ID is not configured in your environment.", 
              variant: "destructive" 
          });
          return;
      }
