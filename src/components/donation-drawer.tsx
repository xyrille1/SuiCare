...
export function DonationDrawer({ open, onOpenChange, campaign: initialCampaign }: DonationDrawerProps) {
  const { getCampaignById, isLoaded, donateToCampaign, isPending } = useCampaigns();
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <div className="relative">
                          ...
                          <Input type="number" placeholder="10" className="pl-9" {...field} step="any" />
                        </div>
                      </FormControl>
                      <FormDescription>
          ...
          {currentAccount?.address === campaign.recipientAddress && (
            <div className="mt-8">
              {campaign.milestones.map((milestone, index) => {
                const isFunded = (campaign.escrowBalance / campaign.goal) * 100 >= milestone.percentage;
