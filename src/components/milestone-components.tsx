...
const SUI_ADMIN_CAP_ID = process.env.NEXT_PUBLIC_SUI_ADMIN_CAP_ID!;
...
export function MilestoneProgressDisplay({ milestones, escrowBalance, goal }: MilestoneProgressDisplayProps) {
    ...
    if (totalPercentage !== 100) {
      return (
        <div className="text-center p-4 rounded-lg bg-destructive/10 text-destructive">
          ...
          <p className="text-sm">The sum of milestone percentages must equal 100%. Current total: {totalPercentage}%.</p>
        </div>
      );
    }
            ...
            return (
                <li key={index} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className={`font-medium ${isFunded ? 'text-foreground' : 'text-muted-foreground'}`}>
                                {milestone.percentage}% - {milestone.description}
                            </p>
                            <p className={`text-sm ${isFunded ? 'text-foreground' : 'text-muted-foreground'}`}>
                                Status: {getMilestoneStatusText(milestone.status)}
                            </p>
                        </div>
                    </div>
                    <p className={`font-semibold text-sm ${isFunded ? 'text-foreground' : 'text-muted-foreground'}`}>
...
export function RequestReleaseButton({ campaignId, milestoneIndex, creatorAddress }: RequestReleaseButtonProps) {
    ...
    const handleRequest = () => {
        ...
        signAndExecute(
            ...
            {
                onSuccess: (result) => {
                    toast({ title: "Success", description: `Milestone release requested. Digest: ${result.digest.slice(0, 10)}...` });
                    refetchCampaigns();
                },
                onError: (error) => {
    ...
    return (
        <Button onClick={handleRequest} disabled={isPending} className="w-full mt-4">
            {isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
            {isPending ? "Requesting..." : `Request Release for Milestone #${milestoneIndex + 1}`}
        </Button>
    );
}
...
export function AdminVerificationPanel({ campaignId, milestones, adminAddress }: AdminVerificationPanelProps) {
    const { mutate: signAndExecute, isPending } = useSignAndExecuteTransactionBlock();
    const { toast } = useToast();
    const { refetchCampaigns } = useCampaigns();
    ...
    const handleVerification = (milestoneIndex: number) => {
        ...
        signAndExecute(
            ...
            {
                onSuccess: (result) => {
                    toast({ title: "Success", description: `Milestone verified and funds released. Digest: ${result.digest.slice(0, 10)}...` });
                    refetchCampaigns();
                },
                onError: (error) => {
                    toast({ title: "Error", description: error.message, variant: "destructive" });
        ...
        <div className="mt-8 p-4 border-l-4 border-primary bg-primary/10">
            ...
            <div className="space-y-3">
                {requestedMilestones.map(m => (
                    <div key={m.index} className="flex items-center justify-between p-3 rounded-md bg-background">
                        <div>
                            <p className="font-semibold">Milestone #{m.index + 1}: {m.description}</p>
                            <p className="text-sm text-muted-foreground">Release of {m.percentage}% of funds requested.</p>
                        </div>
                        <Button onClick={() => handleVerification(m.index)} disabled={isPending} size="sm">
                            {isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                            Verify & Release
                        </Button>
                    </div>
